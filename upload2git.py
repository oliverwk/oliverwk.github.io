...
data = {"message": str("updated {} from Github Actions on {} using the RAW Github api ".format(file, datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S"))) , "content": base64.b64encode(rHtml.encode('ascii')).decode('utf-8'), "sha": str(json.loads(content_res.text)["sha"])}
